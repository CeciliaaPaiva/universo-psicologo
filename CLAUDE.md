...
### Perfil do paciente e anamnese *(ajuste 07/07/2026 — implementado integralmente em v0.5.1/v0.6.1)*
...
- **Anamnese básica (preenchimento em v0.5.1; acesso do psicólogo em v0.6.1):** já fez terapia antes, motivo de buscar terapia agora, se toma medicação controlada, contato do responsável (condicional a menor de idade). Cifrada com `CriptografiaService` (AES-256-GCM), módulo `usuario.domain.Anamnese`.
  - **A anamnese é sempre do paciente — nunca pública, nunca permanente para o psicólogo.** O acesso do psicólogo (`GET /api/usuarios/psicologo/pacientes/{pacienteId}/anamnese`, `AnamneseService.buscarParaPsicologo`) é **temporário**: liberado enquanto existir uma `Sessao` `AGENDADA` entre os dois, revogado assim que ela vira `REALIZADA` (checagem ao vivo a cada requisição, sem flag persistida de "acesso concedido"). **Adaptação de 14/07/2026 em relação à regra original do backlog:** a US-030 condicionava a liberação a uma `Cobranca` paga associada à sessão ainda `AGENDADA`, mas no modelo financeiro (Sprint 5) a cobrança só é gerada quando a sessão vira `REALIZADA` — essa combinação nunca ocorreria. Decisão com o stakeholder: liberar por sessão `AGENDADA`, sem exigir cobrança paga, mantendo a mesma janela pretendida.
  - Se o paciente agendar depois com outro psicólogo, esse novo profissional ganha sua própria janela de acesso (sessão `AGENDADA` *com ele*), independente — não é um acesso permanente nem compartilhado entre profissionais.
  - Segue o mesmo padrão de proteção do prontuário: conteúdo cifrado com `CriptografiaService` (AES-256-GCM) e toda leitura registrada em auditoria — tabela `auditoria_anamnese`, `AuditoriaAnamnese`, no espírito de `AuditoriaProntuarioService`, mas só com ação de leitura (controle de acesso é por janela de tempo, não por autoria fixa).
...
### Mensagens internas *(nova feature, ajuste 07/07/2026, implementada em v0.6.1)*
- Chat interno entre psicólogo e paciente, módulo `mensagem` (`Mensagem`, `MensagemService`, `MensagemController`, `GET/POST /api/mensagens/{outroId}`, `GET /api/mensagens/contatos`).
- **Liberado quando existir uma `Cobranca` com status `PAGO` entre os dois** (`CobrancaRepository.existsPagaEntrePacienteEPsicologo`) — checado a cada leitura/envio, não só na primeira mensagem. O canal é por **relação paciente↔psicólogo**, não por sessão individual: uma vez aberto, permanece aberto (interpretação prática — travar por sessão tornaria o chat inutilizável para acompanhamento contínuo).
- **Mensagens não são cifradas** (ao contrário de prontuário/anamnese) — critério de aceitação de US-029 não exige criptografia; reavaliar antes de produção dado que pode conter conteúdo sensível.
- Notificação de mensagem nova é só in-app (`NotificacaoService`), não dispara e-mail — evita volume de e-mail por mensagem de chat.
